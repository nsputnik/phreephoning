...
What if you could pick up an old-school telephone in your house, call a friend’s house across town, the country or wrold, and have that call travel over your existing internet connection, fully encrypted, with no monthly bill from a phone company? That’s the basic idea behind this project.  Phreephoneing is a free, private phone system built from Raspberry Pis, Analog Telephone Adapters (ATAs), and a router that creates an encrypted mesh between locations.

# WRT Powered Subnet Router / Tailscale Setup Guide for VoIP ATAs

This guide covers setting up a GL.iNet Brume 2 router with Tailscale to connect remote ATAs to a central PBX for free, private and encrypted phone system.  It is not connected to the larger phone system. You can only call the users you set up with the system.  You don't have to pay a phon company monthly, just your ISP.  Wireguard encrypts the traffic between subnet routers, just like the major data centers do.  Feel free to share this file with ChatGPT or Claude Code for it to be an interactive guide.
...
7. In Brume web UI: **VPN → Tailscale**
...
> **IMPORTANT: Configure before deployment!** Set up and join each remote Brume to your Tailnet **before** shipping it to the remote location. This allows you to SSH into the remote Brume 2 via Tailscale (computer or smartphone clients) for any troubleshooting or configuration changes after deployment.
...
8. In Brume web UI: **VPN → Tailscale**
...
1. Ship or carry the Brume to the remote location
...
If anything goes wrong, you can SSH to the Brume via its Tailscale IP from the computer that you installed the Tasilscal client app. Make sure Tailscale is turned on,and you also have to log in for each session.
...
7. Click **Scan** to find available WiFi networks
8. Select the remote site's WiFi network and enter the password
9. Click **Join** - the Beryl will connect wirelessly to the existing router for internet access
10. Verify the connection shows as active in the Repeater section

### Configure Tailscale and Firewall

Once connected to WiFi, configure Tailscale the same way as the Brume 2:

11. Go to **VPN → Tailscale** and enable it
...
1. Ship the pre-configured Beryl AX to the remote location
...
The following sections contain the detailed commands referenced by Steps 1A, 1B, and 1C above.
...
1. Access the ATA's web admin (e.g., http://192.168.10.100)
