...
**Symptom**: `tailscale ping 192.168.1.142` returns "no matching peer"
...
Check if the main Brume is advertising 192.168.1.0/24:
```bash
# On main Brume
tailscale debug prefs | grep -A3 AdvertiseRoutes
```
...
2. Find the main Brume (gl-mt2500)
3. Verify 192.168.1.0/24 is listed and **approved** (not "awaiting approval")
...
**Symptom**:
- `ping 192.168.1.142` from Brume works
...
```bash
tailscale debug netmap 2>&1 | grep "192.168.1.0"
```
...
**Symptom**: Remote Brume has same subnet (192.168.1.x) on its WAN as the PBX network

**Cause**: Tailscale can't distinguish between local and remote 192.168.1.x addresses
...
# Watch specific host
tcpdump -i any host 192.168.1.142 -n
```
...
```
1. ATA (192.168.9.217) sends REGISTER to 192.168.1.142:5060
2. → Gateway (192.168.9.1 = Brume br-lan)
...
6. → Forwards to ATA (192.168.9.217)
```
